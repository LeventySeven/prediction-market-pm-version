...
export default function HomePage({
  ...
  const [enabledProviders, setEnabledProviders] = useState<Array<"polymarket" | "limitless">>(() =>
    sanitizeEnabledProviders(initialEnabledProviders ?? bootstrapRef.current?.enabledProviders)
  );
  const [enabledProvidersResolved, setEnabledProvidersResolved] = useState<boolean>(
    Boolean(initialEnabledProviders ?? bootstrapRef.current)
  );
  ...
  const initialSelectedMarketDataRef = useRef<{
    ...
    marketId: initialSelectedMarketId,
    candles: initialMarketCandles,
    publicTrades: initialMarketPublicTrades,
    liveActivityTicks: initialMarketLiveActivityTicks,
  ...
  const [currentView, setCurrentView] = useState<ViewType>(() => initialView ?? getViewFromLocation());
  ...
  const [marketChartRange, setMarketChartRange] = useState<MarketChartRange>("1M");
  const [marketCandles, setMarketCandles] = useState<PriceCandle[]>(initialMarketCandles);
  const [marketPublicTrades, setMarketPublicTrades] = useState<PublicTrade[]>(initialMarketPublicTrades);
  ...
  const getAggregatedCatalogResult = useCallback((params: CatalogFetchParams): CatalogFetchResult => {
    ...
    for (let page = 1; page <= params.page; page += 1) {
      ...
      for (const row of entry.rows) {
        const key = String(row.canonicalMarketId ?? `${row.provider ?? "polymarket"}:${row.id}`).trim();
  ...
  const loadMarkets = useCallback(async () => {
    ...
    if (!hasActiveKeyData) {
      ...
      setMarketsLoadingMessage(null);
      if (hasCached) {
        setHasNextCatalogPage(cachedAggregate.hasMore);
        setMarkets(cachedAggregate.rows.map((m) => mapMarketApiToMarket(m, lang)));
  ...
  useEffect(() => {
    ...
    if (!sentinel) return;
    if (currentView !== "CATALOG" || selectedMarketId) return;
  ...
  const catalogMarkets = useMemo(() => {
    const parseVol = (m: Market) =>
      resolveDisplayVolume(m.totalVolumeUsd ?? m.volumeRaw).raw ?? 0;
  ...
  useEffect(() => {
    ...
    const startSelectedMarketFallbackPoll = () => {
      let stopped = false;
  ...
  useEffect(() => {
    ...
    const hasPrefetchedSelectedMarket =
